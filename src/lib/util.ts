import countryPhoneData from '../phone-data/country_data';
...
export function findCountryPhoneDataByCountry(country: string) {
	// if no country provided, assume it's TURKEY
	if (!country) {
		return countryPhoneData.find(countryPhoneDatum => countryPhoneDatum.alpha3 === 'TUR') || null;
	}
